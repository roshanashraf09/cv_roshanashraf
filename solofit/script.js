...
(function () {
  ...
  // Storage key is scoped by Firebase user UID once logged in,
  // so different accounts on the same browser don't share data.
  function getStorageKey() {
    if (window.fbAuth && window.fbAuth.currentUser) {
      return 'solofit_v1_' + window.fbAuth.currentUser.uid;
    }
    return 'solofit_v1';
  }
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // Seed state if first login on this device
        if (!loadState()) {
          const displayName = cred.user.displayName || email.split('@')[0];
          saveState(defaultState(displayName));
        }
        ...
        const msg = err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential'
          ...
          ? 'No hunter found with this email'
  ...
  if (signupForm) {
    signupForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // Store handle as display name on the Firebase user
        ...
        const msg = err.code === 'auth/email-already-in-use'
          ? 'A hunter is already registered with this email'
          : err.code === 'auth/weak-password'
          ? 'Password too weak'
          : err.code === 'auth/invalid-email'
          ? 'Invalid email address'
          : err.message || 'Awakening failed';
        setError('email', msg);
        btn.disabled = false;
        btn.innerHTML = original;
      }
  ...
  if (forgot) {
    forgot.addEventListener('click', async (e) => {
      ...
      const email = $('email').value.trim();
      if (!email) {
        setError('email', 'Enter your email first, then click forgot password');
        return;
      }
      try {
        ...
        showToast('Could not send recovery email. Check the address.');
      }
    });
  }
  ...
  // ============================================================
  // PROTECTED PAGES — wait for Firebase auth state before running
  // ============================================================
  const isProtected = document.body.classList.contains('dash-page') ||
                      document.body.classList.contains('profile-page');

  if (!isProtected) return;

  // Wait for Firebase to tell us if we have a logged-in user
  if (!window.fbAuth) {
    // Firebase didn't load — fall back to demo mode (localStorage as before)
    runProtectedPages();
    return;
  }
